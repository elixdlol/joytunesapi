...
function GreenhouseRequest(company_name, api_key) {
	...
	GH_APPLICATIONS =
		'https://' + api_key + ':@api.greenhouse.io/v1/applications/';

	var api_uri = API_ROOT + company_name + '/';
	var module = {};

	function _validateEndpoint(endpoint, params) {
		if (!(endpoint in GH_ENDPOINTS)) {
			throw new Error(
				"'" + endpoint + "' is not supported by Greenhouse"
			);
		}

		if (
			endpoint in ['office', 'job', 'department'] &&
			!params.hasOwnProperty('id')
		) {
			throw new Error(
				"'" + endpoint + "' requires id parameter to be set"
			);
		}
